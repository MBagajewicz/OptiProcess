...
from Aircooler.Calculations import(
    Calculations_aircooler_f_base,
    Calculations_aircooler_Fconfig
)
from Common_Equations_HEX import Calculations_HEX_Consistency
import numpy as np
from Aircooler.Model.Model_Def_Aircooler import Model_Aircooler
import sys
# endregion
##################################################################################################################
...
def Fconfig(m_p):
    F_base = Calculations_aircooler_f_base.f_base(m_p['Thi'], m_p['Tho'], m_p['Tci'], m_p['Tco'], m_p['mh'], m_p['Cph'])
    F_config = Calculations_aircooler_Fconfig.aircooler_Fconfig(F_base)
    ppFconfig = np.array(list(F_config.values()))
    m_p['ppF'] = ppFconfig
    #print(m_p['ppF'])
    return m_p
...
def consistency(m_d, m_p, save_result):
    ...
    def variables_bounds(m_d):
        m_i = Model_Aircooler['Model_Info']
