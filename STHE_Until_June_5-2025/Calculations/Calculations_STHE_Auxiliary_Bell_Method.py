...
def STHE_Ltb_func(Ds, dte, m_p):
    ...
    Ltb[CondInter] = 0.4e-3
        ...
        #
        # Cond_1[dte <=31.75e-3] = True
        # Cond_2[(68 * (dte * 1e3) + 228) > 900] = True
        # CondInter = np.logical_and(Cond_1, Cond_2)
        #
        # Ltb[CondInter] = 0.4e-3
    #else:
     #   Ltb = np.array(m_p['Ltb_g'])
    #print('Ltb',Ltb)
...
def STHE_shellside_Sm(Ds, dte, rp, lay, L, Nb, m_p):
    ...
    # Cross flow area considering the hydraulic impact // Área de escoamento cruzado considerando o impacto hidráulico da deposição
...
def STHE_shellside_Ntcc(Ds, dte, rp, lay, Bc):
    # Description // Descrição
    # Number of tube rows in cross flow (without window tubes) // Filas de tubos em escoamento cruzado (sem os tubos nas janelas)

    # Tube pitch // Passo nos tubos
    Ltp = rp * dte

    # Tubes distance in the flow direction // Distância entre os tubes na direção do escoamento
    Lpp = Ltp  # Lpp for square bundle // Lpp para matriz quadrada
    Lpp[lay == 2] = 0.866 * Ltp[lay == 2]  # Lpp for triangular bundle // Lpp para matriz triangular
    Lpp[lay == 3] = 0.707 * Ltp[lay == 3]  # Lpp for rotate triangular bundle // Lpp para matriz triangular rodado
...
def STHE_shellside_Ntcw(Ds, dte, rp, lay, Bc, m_p):
    # Description // Descrição
    # Number of tube rows in window region // Número efetivo de filas de tubos na região das janelas
    ...
    # Tubes distance in the flow direction // Distância entre os tubes na direção do escoamento
    Lpp = Ltp  # Lpp para matriz quadrada
    Lpp[lay == 2] = 0.866 * Ltp[lay == 2] # Lpp para matriz triangular
    Lpp[lay == 3] = 0.707 * Ltp[lay == 3]  # Lpp para matriz triangular
...
def STHE_shellside_Jb1(Ds, dte, Npt, rp, lay, ms, ros, mis, L, Nb, Bc, m_p):
    ...
    # Razão entre número de tiras de selagem e filas de tubos // The division of number of sealing strips, Nss, by the
    # number of tube rows crossed between baffle tips in one baffle section
    rss = m_p['Nss'] / Ntcc
    #print('rss',rss)
    ...
    #Sb = STHE_shellside_Sb(Ds, dte, rp, m_p)
    Sb = STHE_shellside_Sb(Ds, L, Nb, m_p)
