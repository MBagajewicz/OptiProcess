...
from RTH.Calculations import (
    ...
    Calculations_RTH_Geometry,Calculations_RTH_Flow
)
from math import pi
import numpy as np
#endregion
...
def Kettle_Nusselt_tubeside(rol,rov,fluid_type, Cpt, mil,miv, kl_t, thk, dte,Ds,Npt, rp, lay, m_t):
    ...
    Ret = Calculations_RTH_Flow.fun_Ret(Ds, dte,Npt, rp, lay, m_t, rol,rov,fluid_type, mil,miv, thk)
    if fluid_type==1:
        Prt = Cpt * mil / kl_t
    elif fluid_type==2:
        Prt = Cpt * miv / kl_t
...
def fun_Fb(Ds, dte, Npt, rp, lay):
    Db = Calculations_RTH_Geometry.fun_Db(Ds,Npt)
    Klay = Calculations_RTH_Geometry.fun_Klay(lay)
...
def fun_ht(Ds, dte, Npt, rp, lay, L, rol, rov, g, k_t, m_t, mil, miv, fluid_type,thk,kl_t,Cpt):
    Ntt=Calculations_RTH_Geometry.fun_Ntt(Ds, dte, Npt, rp, lay)
    if fluid_type == 1:
...
def fun_U(Ds, dte, Npt, rp, lay, L, thk, rol, rov, g, k_t, m_t,mil, miv, Q, BR, Pc, Fp, hnc, Rft, Rfs, ktube, fluid_type, kl_t, Cpt,msh,Fv,rol_s,Cpl_s,mil_s,kl_s,miv_s,rov_s):
    dti = Calculations_RTH_Geometry.fun_dti(dte,thk)
