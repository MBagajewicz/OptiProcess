...
from SPHE_DT.Calculations import Calculations_SPHE_DT_Reynolds
#endregion
...
def SPHE_h(*args):
    ...
    if len(args) == 14:
        ...
        Dh = 2 * dh * H / (dh + H)
        Dc = 2 * dc * H / (dc + H)
        Ds = Calculations_SPHE_DT_Reynolds.SPHE_spiral_outer_diameter(L, dh, dc, thk, ds)
        Gh = mh / (dh * H)
        Gc = mc / (dc * H)
        Prh = mih * Cph / kh
        Prc = mic * Cpc / kc
        Reh, Rec, _, _ = Calculations_SPHE_DT_Reynolds.SPHE_Reynolds(dh, dc, H, mh, mc, mih, mic, L, thk, ds)
