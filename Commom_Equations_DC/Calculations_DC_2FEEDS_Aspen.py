...
def fun_initial_Aspen(file_name, stream_name, block_name, comp_name, z_feed, F_feed, T_feed, P_col, x_TOP, x_BOTTOM, 
    ...
    for i in range(len(z_feed)):
        for comp, mole_frac in zip(comp_name, z_feed[i]):       # Molar fractions for components
            molar_flow = F_feed[i]*mole_frac                       # Calculate molar flow for the component
            Aspen.Tree.FindNode(rf'\Data\Streams\{stream_name[i]}\Input\FLOW\MIXED\{comp}').Value = molar_flow     # Input molar flow for each component
        Aspen.Tree.FindNode(rf'\Data\Streams\{stream_name[i]}\Input\TEMP\MIXED').Value = T_feed[i]                    # Input feed stream temperature (K)
        Aspen.Tree.FindNode(rf'\Data\Streams\{stream_name[i]}\Input\PRES\MIXED').Value = P_col                     # Input feed stream pressure (Pa)
...
def fun_run_Aspen(v_Ns, v_Nf1, v_Nf2, Aspen, block_name, stream_names, components, v_Nc): 

    feed1_name = stream_names[0]
    feed2_name = stream_names[1]
    distillate_name = stream_names[2]

    # Update RadFrac inputs for current candidate:
    Aspen.Tree.FindNode(rf'\Data\Blocks\{block_name}\Input\NSTAGE').Value = v_Ns
    Aspen.Tree.FindNode(rf'\Data\Blocks\{block_name}\Input\FEED_STAGE\{feed1_name}').Value = v_Nf1
    Aspen.Tree.FindNode(rf'\Data\Blocks\{block_name}\Input\FEED_STAGE\{feed2_name}').Value = v_Nf2
    ...
    if is_error == 0:
        print(f'Simulation converged successfully for Ns = {v_Ns}, Nf1 = {v_Nf1} and Nf2 = {v_Nf2}')
        ...
        if message == 'completed with warnings:':
            ...
            print(f'Aspen Simulation for candidate Ns = {v_Ns} Nf1 = {v_Nf1} and Nf2 = {v_Nf2} was completed with warnings')
            ...
            print(f'Simulation did NOT converged for Ns = {v_Ns} Nf1 = {v_Nf1} and Nf2 = {v_Nf2}','Reseting and trying again')
        ...
        if is_error == 0:
            ...
            print(f'Simulation did NOT converged for Ns = {v_Ns} Nf1 = {v_Nf1} and Nf2 = {v_Nf2} on the second try, reseting for next run')
            Aspen.Reinit()
