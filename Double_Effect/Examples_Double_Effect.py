...
Example1 = {
    ...
    'Equipment1': {
        ...
        'Model_Parameters': {
            ...
            'F_f' :  550.0,             # Feed flow (kmol/h)
            #'Split' : 0.7,             # Split fraction of the molar flow in the first column
        ...
        'Equipment1': {
            ...
            'Model_Declarations': {
                ...
                'Discrete_Values_of_Variables': [
                            ...
                            list(range(10, 20)), # Nf (Feed considered from stage 3 to 40, considering there are always a stripping 
                                                # and a rectifying section)
                                                # Nf is interpreted regarding stages and not trays (same as in Aspen Plus)
            ...
            'F_f' :  100.0,                     # Feed flow (kmol/h)
            #'Feed' : 100,                       # Same Feed flow in the first column (kmol/h)
            #'Split' : 0.7,                      # Same Split fraction of the molar flow in the first column
            'T_f' :  113.4 + 273.15,            # Feed temperature (K)
            # Separation Task Specification 
...
###################################################################################################################
# region INPUT EXAMPLE 2 - METHANOL-WATER DISTILLATION COLUMN --> Type : Double_Effect + DC_Q_Condenser - Bigger Range
    ...
    'Equipment1': {
        ...
        'Model_Declarations': {
            ...
            'Discrete_Values_of_Variables': [
                            ...
                            list(range(5, 22)), # Nf (Feed considered from stage 3 to 40, considering there are always a stripping 
                                                # and a rectifying section)
                                                # Nf is interpreted regarding stages and not trays (same as in Aspen Plus)
                            list(range(23, 24))  # Ns (Stages and not trays, as in Aspen Plus)
                                                # Ns = 5 means: condenser + 3 stages within the column + reboiler
                                                # Ns = 42 means: condenser + 40 stages within the column + reboiler                  
                                    ],
        ...
        'Equipment1': {
            ...
            'Model_Declarations': {
                ...
                'Discrete_Values_of_Variables': [
                            ...
                            list(range(5, 20)), # Nf (Feed considered from stage 3 to 40, considering there are always a stripping 
                                                # and a rectifying section)
                                                # Nf is interpreted regarding stages and not trays (same as in Aspen Plus)
                            list(range(13, 20))  # Ns (Stages and not trays, as in Aspen Plus)
                                                # Ns = 5 means: condenser + 3 stages within the column + reboiler
                                                # Ns = 42 means: condenser + 40 stages within the column + reboiler           
                ],
...
# region INPUT EXAMPLE 3 - METHANOL-WATER DISTILLATION COLUMN --> Type : Double_Effect + DC_Q_Condenser (more stages in HPC)
    ...
    'Equipment1': {
        ...
        'Model_Declarations': {
            ...
            'Discrete_Values_of_Variables': [
                            ...
                            list(range(18,21)), # Nf (Feed considered from stage 3 to 40, considering there are always a stripping 
                                                # and a rectifying section)
                                                # Nf is interpreted regarding stages and not trays (same as in Aspen Plus)
                            list(range(16, 25))  # Ns (Stages and not trays, as in Aspen Plus)
                                                # Ns = 5 means: condenser + 3 stages within the column + reboiler
                                                # Ns = 42 means: condenser + 40 stages within the column + reboiler                  
                                    ],
...
#######################################################################
    ...
    'Next_Level_Equipments': {
        ...
        'Equipment1': {
            ...
            'Nsmin' : 13,                       # Minimum number of stages (Condenser + 11 stages within the column + Reboiler)
            'Nfmin' : 3,                        # Minimum feed tray
            'Pcol' : 3e5,                       # Column Pressure (Pa) - constant throughout the column --> Pendência: consider some pressure drop
            # Feed Data 
            'z_f' : [0.6, 0.4],                 # Feed molar composition [Benzene, Toluene, m-Xylene]
            'F_f' :  100,                       # Feed flow (kmol/h)
            #'Feed' : 100,                       # Same Feed flow in the first column (kmol/h)
...
###################################################################################################################
# region INPUT EXAMPLE 4 - METHANOL-WATER DISTILLATION COLUMN --> Type : Double_Effect + DC_Q_Condenser
