...
def TAC_OF(Nf_c,Ns_c,m_p):
    ...
    TAC_OF.count_LPC += 1
    print(f"\033[92m[LPC Run #{TAC_OF.count_LPC}] Aspen simulation executed.\033[0m")
    ...
    if results:
        ...
        OPEX_COL = Cooling_Cost # Considering only the cooling water used
...
def LB_Gen(Nf_c,Ns_c,m_p):
    ...
    # Initializing function call count attribute # Tomazim
    if not hasattr(LB_Gen, "count_LB_LPC"):
        LB_Gen.count_LB_LPC = 0
    ...
    for Nf in range(Nfmin,Nfmax+1):    # For Nf from Nfmin to Nsmax - 2
        ...
        # Increment function call count # Tomazim
        LB_Gen.count_LB_LPC += 1
        print(f"\033[92m[LB_LPC Run #{LB_Gen.count_LB_LPC}] Aspen simulation executed.\033[0m")
        ...
        if GD_results[Nf]:
            ...
            # Call Set_Up_HPCol
            from Double_Effect.Model import Parameters_Update_Double_Effect
            LB_Gen_HPCol = {}
            Reb_duty = GD_results[Nf]['reboiler_duty']
            LB_Gen_HPCol[Nf] = Parameters_Update_Double_Effect.Set_Up_LB_HPCOL(Ns_c, Nf_c, m_p['Aspen_engine'], Reb_duty, m_p)
            GD_Solution_Within[Nf] = LB_Gen_HPCol[Nf]
            TAC_COL_HPCol = LB_Gen_HPCol[Nf][0]  # Taking only the TAC_best value 
            print(f'-------LB_TAC (HPCol) = {TAC_COL_HPCol:.2f}')
            '''
            # Running High Pressure Column Design:
            ...
            GD_TAC[Nf] = (1/m_p['Pb'])*(CAPEX_COL + CAPEX_COND + CAPEX_REB + CAPEX_RD) + OPEX_COL # Low Pressure Column TAC - CAPEX_REB (because is the same as HPCol condenser)
            print(f'-------LB_TAC (LPCol) = {GD_TAC[Nf]:.2f}')
            GD_TAC[Nf] = GD_TAC[Nf] + TAC_COL_HPCol  # Adding High Pressure Column TAC to the list
            GD_HE_TAC[Nf] = (1/m_p['Pb'])*(CAPEX_COND + CAPEX_REB + CAPEX_RD) + OPEX_COL + TAC_COL_HPCol  # HPC_LB_TAC to be used in lower bounds generation
    ...
    Solution_Within_best = GD_Solution_Within.get(Arg_best[0], None) #TOMAZIM
    ...
    try:
        ...
        df_matrix = pd.DataFrame(index=Ns_vals, columns=Nf_vals)
        ...
        folder = os.getcwd()
        base_name = "LowerBound_Matrix_DoubleEffect_"
    ...
    # ------------------------------
    '''
    # Tomazim - LB_Gen does not provide a True Solution, only the LB value (so 'Fobj_within_LB' is False)
    # If 'Fobj_within_LB' : True (return all arguments) --> return [LB_sol, TAC_best, Arg_best, Solution_Within_best] 
    # IF 'Fobj_within_LB' : False (return only LB values) --> return LB_sol 
