...
def F_reb(Ds, dte, Npt, rp, lay, L, msh, f_v, rol_s, mil_s):
    Re = Calculations_HTSR_Heat_Transfer.fun_Re_shell(Ds, dte, Npt, rp, L, msh, f_v, rol_s, mil_s)
...
##################################################################### Perda de carga RF
def V_RF( D_RF,msh,f_v, rov_s, rol_s):
    rho_tp2 = ro_tp2(f_v, rov_s, rol_s)
    V_RF =4 * msh / (np.pi * rho_tp2 * D_RF**2)
    return V_RF

def deltaP_RF( D_RF,msh, f_v,rol_s, rov_s, miv_s, mil_s, L_RF):
    ...
    deltaP_RF= phi*(f_RF * L_RF * G_RF**2) / (2000 * D_RF * SG2)

    return deltaP_RF


##################################################################### Perda de carga RS
def V_RS( Ds,Npt,L,D_FF,msh,f_v, rov_s, rol_s):
    DRS =D_RS(Ds,Npt,L,D_FF)
    rho_tp2 = ro_tp2(f_v, rov_s, rol_s)
    V_RS =4 * msh / (np.pi * rho_tp2 * DRS**2)
    return V_RS

def deltaP_RS(Ds,Npt,L,D_FF,msh, f_v,rol_s, rov_s, miv_s, mil_s, L_RS ):
    DRS =D_RS(Ds,Npt,L,D_FF)
    phi =phi_LO(rol_s, rov_s, miv_s, mil_s, f_v)
    rho_tp2 = ro_tp2(f_v, rov_s, rol_s)
    SG2 =rho_tp2/1000
    ...
    deltaP_RS= phi*(f_RS * L_RS * G_RS**2) / (2000 * DRS * SG2)
...
def deltaH_L(Ds,dte,Npt,rp,lay,L, D_FF, D_RF,msh, mil_s, L_FF,f_v,rol_s, rov_s, miv_s,L_FS,L_RS,L_RF,g):
    ...
    d = deltaP_RF(D_RF,msh, f_v,rol_s, rov_s, miv_s, mil_s, L_RF)
    e = deltaP_RS(Ds,Npt,L,D_FF,msh, f_v,rol_s, rov_s, miv_s, mil_s, L_RS )
    f = deltaP_mo(Ds,dte,Npt,rp,L,msh,f_v, rol_s, rov_s, miv_s, mil_s)
    X = a + b + c +d + e+f 
    rho_tp1 =ro_tp1(f_v, rov_s, rol_s)
    rho_tp2=ro_tp2(f_v, rov_s, rol_s)
    deltaH_L =(X +Ds*g*(rho_tp1-rho_tp2))/(g*(rol_s-rho_tp2))
    print('hl =',deltaH_L)
    return deltaH_L
