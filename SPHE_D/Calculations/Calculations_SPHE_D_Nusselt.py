...
#region Calculations
def SPHE_Nusselt(Re, Pr, Ds, Dh, L):
    """
    Calculate the Nusselt number for scalar or NumPy array inputs.
    """

    Re, Pr, Ds, Dh, L = np.broadcast_arrays(
    ...
    if np.any(Ds <= 0):
        raise ValueError("Ds must be greater than zero.")
    if np.any(L <= 0):
    ...
    Reynolds_Critical = 20000.0 * (Dh / Ds) ** 0.32
    ...
    Nu[turbulent] = (
        0.023
        * (1.0 + 3.54 * Dh[turbulent] / Ds[turbulent])
