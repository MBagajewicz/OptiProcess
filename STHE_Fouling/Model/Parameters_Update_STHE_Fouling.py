...
#   0.1          01-Jul-2025     Augusto Vieira            Inclusion of inlet temperatures on allocation function
...
- Included inlet temperature on allocation 
- Rft will only be taken as constant if initially set on the m_p dicitonary through fouling model = constant
"""
...
def allocation(m_p):
    if m_p['yfluid'] == 'cold_stream':
        ...
        # Assign Rft and ft_thk if fouling is constant and Rfc is present
        if m_p.get('Fouling_Method') == 'constant' and 'Rfc' in m_p:
            m_p['Rft'] = m_p['Rfc']
        ...
        # Assign Rft if fouling is constant and Rfh is present
        if m_p.get('Fouling_Method') == 'constant' and 'Rfh' in m_p:
            m_p['Rft'] = m_p['Rfh']
...
def consistency(m_d, m_p, save_result):
    ...
    def verification_Tco_Thi_STHE(m_p, m_d):
        ...
        if 'Tco' in m_p and 'Thi' in m_p and 'Tho' in m_p:
            ...
            Tco = m_p['Tco']
            Tho = m_p['Tho']
            deltaTmin = m_p.get('DeltaT_min', 5)  # default fallback if not present
            ...
            if Tco < Thi - deltaTmin:
                if Tco > Tho - deltaTmin:
                    ...
                    # Ensure 'Discrete_Variables' and 'Npt' keys exist before assignment
                    if 'Discrete_Variables' not in m_d:
                        m_d['Discrete_Variables'] = {}
                    m_d['Discrete_Variables']['Npt'] = [1]
            else:
                save_result('Error data consistency: Tco > Thi - DeltaTmin\n')
                sys.exit()
        else:
            save_result('Missing required temperature keys: Thi, Tco, or Tho\n')
            sys.exit()
