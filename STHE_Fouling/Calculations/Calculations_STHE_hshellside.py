...
def STHE_h_shellside(ms, ros, Cps, mis, ks, Ds, dte, Npt, rp, lay, L, Nb, Bc, m_p):
    if m_p['Shell_Method'] == "Bell":
        ...
        #print('Jb', Jb1)
        #Jr = Calculations_STHE_Auxiliary_Bell_Method.STHE_shellside_Jr(Ds, dte, Npt, rp, lay, L, Nb, ms, ros, mis, m_p)
        ...
        if np.isscalar(lay):
            K_Deq = 3.46 if lay == 2 else 4.0
        else:
            K_Deq = 4 * np.ones_like(lay, dtype=float)
            K_Deq[lay == 2] = 3.46
        ltp = rp * dte
