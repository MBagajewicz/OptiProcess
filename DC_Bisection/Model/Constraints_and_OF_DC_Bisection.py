...
def Bisection(Ns_c, m_p):
    ...
    a = Nsmin
    b = Nsmax
    tol = abs(m_p['SPEC_1']) # Reference value for condenser duty
    m_p['SPEC_1'] = m_p['Purity'] # Setting SPEC_1 as x_top
    
    # Running Aspen simulation:
    print(f'Running Aspen simulation for candidate with Ns = {a} and Nf = {round(a/2)}')
    results_a = Calculations_DC_Aspen.fun_run_Aspen(a, round(a/2), m_p['Aspen_engine'], m_p['block_name_2'][0], m_p['stream_names_2'], m_p['Comp_name'], m_p['Nc'],m_p)
    print(f'Running Aspen simulation for candidate with Ns = {b} and Nf = {round(b/2)}')
    results_b = Calculations_DC_Aspen.fun_run_Aspen(b, round(b/2), m_p['Aspen_engine'], m_p['block_name_2'][0], m_p['stream_names_2'], m_p['Comp_name'], m_p['Nc'],m_p)
    max_iter = 10
    i = 0
    a_duty = results_a['condenser_duty']
    b_duty = results_b['condenser_duty']
    for _ in range(max_iter):
        
        Nsmid = (a+b) / 2
        print(f'Running Aspen simulation for candidate with Ns = {round(Nsmid)} and Nf = {round(Nsmid/2)}')
        results_m = Calculations_DC_Aspen.fun_run_Aspen(round(Nsmid), round(Nsmid/2), m_p['Aspen_engine'], m_p['block_name_2'][0], m_p['stream_names_2'], m_p['Comp_name'], m_p['Nc'],m_p)
        m = results_m['condenser_duty']
        ...
        i += 1
    return a,b # Returns the interval [Nsmin,Nsmax] where the condenser duty is within the specified tolerance of SPEC_1
...
def LB_Gen(Nf_c,Ns_c,m_p):
    ...
    #Calling Bisection method to define Nsmin and Nsmax for guide line:
    Nsmin, Nsmax = Bisection(Ns_c, m_p)
    ...
    for Nf in range(Nfmin, Nfmax+1):  
        ...
        #call Set_Up_HPCol
        from DC_Q_Condenser.Model import Parameters_Update_DC_Q_Condenser
        min_condenser_duty = Parameters_Update_DC_Q_Condenser.Set_Up_HPCOL(Nsmax, Nf, m_p['Aspen_engine'],m_p)
