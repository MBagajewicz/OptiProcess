...
from Kettle_4.Calculations import (
    ...
    Calculations_Kettle_4_Geometry,Calculations_Kettle_4_Flow
)
from math import pi
import numpy as np
#endregion
...
def Kettle_Nusselt_tubeside(rol,rov,fluid_type, Cpt, mil,miv, kl_t, thk, dte,Ds,Npt, rp, lay, m_t):
    ...
    Ret = Calculations_Kettle_4_Flow.fun_Ret(Ds, dte,Npt, rp, lay, m_t, rol,rov,fluid_type, mil,miv, thk)
    if fluid_type==1:
        Prt = Cpt * mil / kl_t
    elif fluid_type==2:
        Prt = Cpt * miv / kl_t
...
def fun_Fb(Ds, dte, Npt, rp, lay):
    ...
    Klay = Calculations_Kettle_4_Geometry.fun_Klay(lay)
