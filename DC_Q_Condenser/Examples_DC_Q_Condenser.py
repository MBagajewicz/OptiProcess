...
Example1 = {
    ...
    'Equipment1': {
        ...
        'Model_Parameters': {
            ...
            'SPEC_1' : -7098859.86,			    # Condenser duty (kJ/h) (Negative value means condenser duty)
...
###################################################################################################################
# region INPUT EXAMPLE 14 - Methanol-Water DISTILLATION COLUMN WITH SEGMENTAL SMART ENUMERATION - DIFFERENTS PRESSURES
...
Example14 = {
    ...
    'Equipment1': {
        ...
        'Model_Parameters': {
            ...
            'Nsmin' : 1,                       # Minimum number of stages (Condenser + 11 stages within the column + Reboiler)
            'Nfmin' : 1,                        # Minimum feed tray
            'Pcol' : 6.5e5,                       # Column Pressure (Pa) - constant throughout the column --> Pendência: consider some pressure drop
            # Feed Data 
            'z_f' : [0.6, 0.4],                 # Feed molar composition [Benzene, Toluene, m-Xylene]
            'F_f' :  100,                        # Feed flow (kmol/h)
            'T_f' :  113.4 + 273.15,            # Feed temperature (K)
