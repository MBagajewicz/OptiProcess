...
def FIRED_HEATER_OF(Do_td, Ds_ts, L, Npasses, Ntceil, Nrconv, Nprad, Npconv, Hs, Rpr, Rph, Rpv, pd):
    ...
    Voil_Tube = Calculations_FIRED_HEATER_oil_velocity.HEATER_Voil_Tube(Do, pd['Moil'], Npasses, pd['rho_oil'] )
    Reoil_Tube = Do * Voil_Tube * pd['rho_oil'] / pd['mu_oil']
    Nuoil_Tube = 0.023 * np.power(Reoil_Tube,0.8) * np.power(pd['Pr_oil'],0.33) 
    hctoil_Tube = Nuoil_Tube * pd['k_oil']/Do
    Di_Tube = Calculations_FIRED_HEATER_tubes.HEATER_Di_Tube(Do)
    Urad = 1/(pd['rf_gas'] + Do * np.log(Do/Di_Tube)/(2*pd['ks']) + pd['rf_oil'] * (Do/Di_Tube)+(1/hctoil_Tube)*(Do/Di_Tube))
    ...
    Ems = (pd['e_c1'] * Tfb_rad + pd['e_c2'] * Factor_PL + pd['e_c3'] * np.power(Factor_PL,2) + pd['e_c4'])
    F_ex_p = Ems *(A/(alpha*Acp) - 1)
    F_ex = (pd['f_c1'] * np.log(Ems) + (pd['f_c2']/Ems+pd['f_c3'])*F_ex_p + pd['f_c4'] * np.exp(Ems) + pd['f_c5']*np.power(F_ex_p,1.5) + pd['f_c6'])
    print("---------variables----------")
    print("Mgas       : ", Mgas)
    ...
    print("F_ex_p     : ", F_ex_p)
    print("F_ex       : ", F_ex)
    Reoil_tube = Calculations_FIRED_HEATER_pressure.HEATER_Reoil_Tube(Do, pd['Moil'], Npasses, pd['rho_oil'], pd['mu_oil'])
    Nuoil_tube = Calculations_FIRED_HEATER_Urad.HEATER_Nuoil_Tube(Do, pd['Moil'], Npasses, pd['rho_oil'], pd['mu_oil'], pd['Pr_oil'])
    hctoil_tube = Calculations_FIRED_HEATER_Urad.HEATER_hctoil_Tube(Do, pd['Moil'], Npasses, pd['rho_oil'], pd['mu_oil'], pd['Pr_oil'], pd['k_oil'])
    print("Reoil_tube : ", Reoil_tube)
    print("Nuoil_tube : ", Nuoil_tube)
    print("hctoil_tube: ", hctoil_tube)
    Wconv = Calculations_FIRED_HEATER_boxsize.HEATER_Wconv(Npconv, Npasses, pd['lf'], Do, Rph)
    print("Wconv      : ", Wconv)
