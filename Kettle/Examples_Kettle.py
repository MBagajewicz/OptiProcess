...
Example1 = {
    ...
    'Equipment1': {
        ...
        'Model_Parameters': {
            ...
            'roh': 800,         # Density (kg*m**-3)
            'Cph': 2303,        # Heat capacity (J*(kg*K)**-1)
            'mih': 5e-4,        # Viscosity (Pa*s)
            'kh': 0.14,         # Thermal conductivity (W*(m*K)**-1)
            'Rfh': 0.00017611,  # Fouling factor (m**2*oC*W**-1)
            'DPhdisp': 105e3,   # Available pressure drop (Pa)

            # Cold stream
            'mc': 124.9724,     # Flow rate (kg*s**-1)  # 228.79,
            'Rfc': 0.0,         # Fouling Factor (m**2*oC*W**-1)

            # Data of material
            'kt': 50,          # Thermal conductivity of material (W*(m*K)**-1)
            'kf': 208,         # Fins thermal conductivity
            'thk': 0.002413,   # Tube thickness

            # Selection
            'draft': 1,        # 1 = induced draft - 2 = forced draft

            # Fan system
            'etafan': 0.8,      # Efficiency of fan
            'etasr': 0.6,       # Speed reducer
            'etamotor': 0.5,    # Efficiency of motor
            ...
            'Recmax': 10e5,     # Upper bound on the Reynolds number of cold stream
            ...
            'rocout': 0.6,      # REVISAR ARTIGO # Air outlet density (kg/m**3)
            ...
            #
            'ppDte': np.array([0.0245, 0.0245, 0.0245, 0.0245, 0.0245]),
            #
